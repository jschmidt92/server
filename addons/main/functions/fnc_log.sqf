...
/*
	...
	[COMPONENT_STR, "WARNING", "", true] call sof_server_main_fnc_log;
...
switch (_location) do {
	...
	case 3: {
		if (isNull _target) then {
			[COMPONENT_NAME, "INFO", "Logging: No target was passed, using local machine instead", false] call sof_server_main_fnc_log;
			[QGVAR(log), [_text, _syschatEnabled]] call CFUNC(localEvent);
		} else {
